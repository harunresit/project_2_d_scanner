...
ser_arduino.write("c".encode('utf-8'))   #tablayı başa aldık
global if_ici_sayac
if_ici_sayac = 450
...
class Logic(baseUIWidget, baseUIClass):
    ...
    def initUI(self):
        ...
        self.pixels = self.get_pixel() #get_pixel fonksiyonunun döndürdüğü pix numpy dizisini pixels'e atıyoruz
    ...
    def btn_2_Clicked(self):
        ...
        data = []
        onceki_data = []
        for a in range(if_ici_sayac):
            print(a)
            ser_arduino.write("a".encode('utf-8'))
            #time.sleep(0.02)
            ac = ser_sensor.write(g_measuring_val_3.encode('utf-8'))
            time.sleep(0.02)  #921600 de optimum 0.02
            if (ser_sensor.inWaiting()):
                onceki_data = ser_sensor.readline()
                data.append(onceki_data)
                print("evet")
            else:
                print("hayır")
                data.append(onceki_data) #b':01A;0;0;-3000;0 ;9583\r\n'
                ...
                if k % 2 == 0:
                    ...
                    if (anlik_c - onceki_c)<=0.3:
                        for j in range(round(onceki_cx), round(anlik_cx)+1):
                            image.setPixel(j,img_indis,QColor(round((z_uzunluk[k]*256)/5000),round((z_uzunluk[k]*256)/5000),round((z_uzunluk[k]*256)/5000)).rgb()) 
        ...
        pixmap3 = QPixmap.fromImage(image)
        self.label_3.setPixmap(pixmap3)
        #self.label_3.setPixmap(QtGui.QPixmap("images.jpg"))
    ...
    def btn_1_Clicked(self):
        ...
        ser_arduino.write("b".encode('utf-8'))
